...
public class DestroyAsyncEventQueueCommandDUnitTest {
  ...
  public void destroyAeq_returnsSuccess() {
    ...
    locator.invoke(() -> {
      ...
      Assertions.assertThat(config.getCacheXmlContent()).contains("id=\"queue1\"");
    });
    ...
    locator.invoke(() -> {
      InternalConfigurationPersistenceService service =
          ClusterStartupRule.getLocator().getConfigurationPersistenceService();
      Configuration config = service.getConfiguration("cluster");
      Assertions.assertThat(config.getCacheXmlContent()).doesNotContain("id=\"queue1\"");
    });
  }
  ...
  public void destroyAeq_selectsQueuesByGroup_returnsSuccess() {
    ...
    locator.invoke(() -> {
      ...
      Assertions.assertThat(config1.getCacheXmlContent()).doesNotContain("id=\"queue1\"");
      Configuration config3 = service.getConfiguration("group3");
      Assertions.assertThat(config3.getCacheXmlContent()).contains("id=\"queue3\"");
    });
  }
}
