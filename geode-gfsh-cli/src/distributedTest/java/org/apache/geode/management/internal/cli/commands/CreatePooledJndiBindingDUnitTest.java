...
public class CreatePooledJndiBindingDUnitTest {
  ...
  public void testCreateJndiBinding() throws Exception {
    ...
    VMProvider.invokeInEveryMember(
        () -> AssertionsForClassTypes.assertThat(JNDIInvoker.getNoOfAvailableDataSources()).isEqualTo(0), server1, server2);
    ...
    locator.invoke(() -> {
      ...
      AssertionsForClassTypes.assertThat(jndiBindings.getLength()).isEqualTo(1);
      AssertionsForClassTypes.assertThat(xmlContent).contains("user-name=\"myuser\"");
      AssertionsForClassTypes.assertThat(xmlContent).contains("password=\"mypass\"");
      ...
      AssertionsForClassTypes.assertThat(found).isTrue();
    });

    // verify datasource exists
    VMProvider.invokeInEveryMember(
        () -> AssertionsForClassTypes.assertThat(JNDIInvoker.getNoOfAvailableDataSources()).isEqualTo(1), server1, server2);
    ...
    server1.invoke(() -> {
      AssertionsForClassTypes.assertThat(JNDIInvoker.getNoOfAvailableDataSources()).isEqualTo(1);
    });
  }
}
