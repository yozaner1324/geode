...
public class CreateRegionCommandDUnitTest {
  ...
  public void testCreateRegionWithGoodCompressor() throws Exception {
    ...
    server1.invoke(() -> {
      ...
      Assertions.assertThat(region.getAttributes().getCompressor())
  ...
  public void testCreateRegionWithBadCompressor() throws Exception {
    ...
    server1.invoke(() -> {
      Cache cache = ClusterStartupRule.getCache();
      Region region = cache.getRegion(regionName);
      Assertions.assertThat(region).isNull();
    });
  }
  ...
  public void testCreateRegionWithSubregion() throws Exception {
    ...
    server1.invoke(() -> {
      ...
      Assertions.assertThat(subSubRegion).isNotNull();
    });

    locator.invoke(() -> {
      CacheConfig config = ClusterStartupRule.getLocator().getConfigurationPersistenceService()
          .getCacheConfig("cluster");
      Assertions.assertThat(find(config.getRegions(), regionName)).isNotNull();

      RegionConfig parentConfig = find(config.getRegions(), regionName);
      Assertions.assertThat(find(parentConfig.getRegions(), subRegionName)).isNotNull();

      RegionConfig subRegionConfig = find(parentConfig.getRegions(),
          subRegionName);
      Assertions.assertThat(find(subRegionConfig.getRegions(), subSubRegionName))
          .isNotNull();
    });
  }
  ...
  public void testCreateRegionWithPartitionResolver() throws Exception {
    ...
    server1.invoke(() -> {
      ...
      Assertions.assertThat(resolver.getName()).isEqualTo("TestPartitionResolver");
    });
  }
  ...
  public void ensureOverridingCallbacksFromTemplateDoNotRequireClassesOnLocator() throws Exception {
    ...
    server1.getVM().invoke(() -> {
      ...
      Assertions.assertThat(copy.getAttributes().getKeyConstraint().getName()).isEqualTo(myNewCallback);

      Assertions.assertThat(copy.getAttributes().getValueConstraint().getName()).isEqualTo(myNewCallback);
    });
  ...
  public void ensureOriginalCallbacksFromTemplateDoNotRequireClassesOnLocator() throws Exception {
    ...
    server1.getVM().invoke(() -> {
      ...
      Assertions.assertThat(copy.getAttributes().getCacheLoader().getClass().getName()).isEqualTo(myCallback);

      Assertions.assertThat(copy.getAttributes().getCacheWriter().getClass().getName()).isEqualTo(myCallback);

      Assertions.assertThat(copy.getAttributes().getCompressor().getClass().getName()).isEqualTo(myCallback);

      Assertions.assertThat(copy.getAttributes().getKeyConstraint().getName()).isEqualTo(myCallback);

      Assertions.assertThat(copy.getAttributes().getValueConstraint().getName()).isEqualTo(myCallback);
    });
  ...
  public void testCreateRegionFromTemplateWithAsyncEventListeners() {
    ...
    server1.invoke(() -> {
      ...
      Assertions.assertThat(regionFromTemplate).isNotNull();
      Assertions.assertThat(((InternalRegion) regionFromTemplate).getAsyncEventQueueIds())
          .contains(queueId);
    });
  }
  ...
  public void testCreateRegionFromTemplateWithPartitionResolver() {
    ...
    server1.invoke(() -> {
      Region regionFromTemplate = ClusterStartupRule.getCache()
          .getRegion(regionName + "-from-template");
      Assertions.assertThat(regionFromTemplate).isNotNull();
      Assertions.assertThat(((InternalRegion) regionFromTemplate).getPartitionAttributes()
  ...
  public void createRegionCommandCreateCorrectClusterConfigXml() throws Exception {
    ...
    locator.invoke(() -> {
      ...
      for (RegionShortcut shortcut : shortcuts) {
        Assertions.assertThat(find(actual.getRegions(), shortcut.name()))
