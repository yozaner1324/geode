...
public class DestroyRegionCommandDUnitTest {
  ...
  public void testDestroyLocalRegions() {
    ...
    VMProvider.invokeInEveryMember(() -> {
      ...
      Assertions.assertThat(cache.getRegion("region1")).isNull();
  ...
  public void testDestroyDistributedRegions() {
    ...
    locator.invoke(() -> {
      ...
      Assertions.assertThat(group1Config.getCacheXmlContent())
      ...
      Assertions.assertThat(clusterConfig.getCacheXmlContent())
          .containsOnlyOnce("<region name=\"region1\"")
    ...
    locator.invoke(() -> {
      InternalConfigurationPersistenceService service =
          ClusterStartupRule.getLocator().getConfigurationPersistenceService();
      Configuration group1Config = service.getConfiguration("group1");
      Assertions.assertThat(group1Config.getCacheXmlContent()).doesNotContain("region1");

      Configuration clusterConfig = service.getConfiguration("group2");
      Assertions.assertThat(clusterConfig.getCacheXmlContent()).doesNotContain("region1");
    });
  ...
  public void testDestroyRegionWithSharedConfig() throws IOException {
    ...
    locator.invoke(() -> {
      ...
      Assertions.assertThat(sharedConfig.getConfiguration("cluster").getCacheXmlContent())
          .contains("Customer");
    });
    ...
    VMProvider.invokeInEveryMember(() -> {
      Cache cache = ClusterStartupRule.getCache();
      Assertions.assertThat(cache.getRegion("Customer")).isNull();
    }, server1, server2, server3);
  }
}
