...
public class ListMembersCommandDUnitTest {
  ...
  @Rule
  public GfshCommandRule gfsh = new GfshCommandRule(locator1::getJmxPort, PortType.jmxManager);
  ...
  public void listMembersInLocatorGroup() {
    ...
    Assertions.assertThat(output).contains("Member Count : 2");
    Assertions.assertThat(output).contains("locator-0");
    Assertions.assertThat(output).doesNotContain("server-1");
    Assertions.assertThat(output).doesNotContain("server-2");
    Assertions.assertThat(output).doesNotContain("server-3");
    assertCoordinatorTag(gfsh.getCommandResult());
  }
  ...
  public void listMembersInServerGroupOne() {
    ...
    String output = gfsh.getGfshOutput();
    Assertions.assertThat(output).contains("Member Count : 2");
    Assertions.assertThat(output).contains("server-2");
    Assertions.assertThat(output).contains("server-3");
    Assertions.assertThat(output).doesNotContain("server-4");
  }
  ...
  public void listMembersInServerGroupTwo() {
    ...
    String output = gfsh.getGfshOutput();
    Assertions.assertThat(output).contains("Member Count : 1");
    Assertions.assertThat(output).doesNotContain("server-2");
    Assertions.assertThat(output).doesNotContain("server-3");
    Assertions.assertThat(output).contains("server-4");
  }
  ...
  public void listMembersShouldAlwaysTagTheCoordinatorMember(boolean useCoordinator)
    ...
    gfsh.disconnect();
    gfsh.connectAndVerify(jmxPort, PortType.jmxManager);
  ...
  private void assertCoordinatorTag(CommandResult commandResult) {
    ...
    Assertions.assertThat(ids.stream().filter(string -> pattern.matcher(string).matches())).hasSize(1);
  }
}
