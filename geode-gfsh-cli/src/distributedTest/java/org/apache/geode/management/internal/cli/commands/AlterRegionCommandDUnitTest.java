...
public class AlterRegionCommandDUnitTest {
  ...
  public void alterRegionResetCacheListeners() throws Exception {
    ...
    VMProvider.invokeInEveryMember(() -> {
      ...
      Assert.assertEquals(3, attributes.getCacheListeners().length);

      Assertions.assertThat(Arrays.stream(attributes.getCacheListeners()).map(c -> c.getClass().getName())
    ...
    server1.invoke(() -> {
      RegionAttributes attributes =
          ClusterStartupRule.getCache().getRegion("regionA").getAttributes();
      Assert.assertEquals(0, attributes.getCacheListeners().length);
    });
  }
  ...
  public void alterEntryIdleTimeExpiration() {
    ...
    server1.invoke(() -> {
      ...
      Assertions.assertThat(expiry.getTimeout()).isEqualTo(10);
      Assertions.assertThat(expiry.getAction()).isEqualTo(ExpirationAction.INVALIDATE);
    });
    ...
    server1.invoke(() -> {
      ...
      ExpirationAttributes expiry = region.getAttributes().getEntryIdleTimeout();
      Assertions.assertThat(expiry.getTimeout()).isEqualTo(10);
      Assertions.assertThat(expiry.getAction()).isEqualTo(ExpirationAction.DESTROY);
    });
    ...
    server1.invoke(() -> {
      Region region = ClusterStartupRule.getCache().getRegion("regionA");
      ExpirationAttributes expiry = region.getAttributes().getEntryIdleTimeout();
      Assertions.assertThat(expiry.getTimeout()).isEqualTo(5);
      Assertions.assertThat(expiry.getAction()).isEqualTo(ExpirationAction.DESTROY);
    });
  }
  ...
  public void alterEntryIdleTimeExpirationAction() {
    ...
    server1.invoke(() -> {
      Region region = ClusterStartupRule.getCache().getRegion("regionA");
      ExpirationAttributes expiry = region.getAttributes().getEntryIdleTimeout();
      Assertions.assertThat(expiry.getTimeout()).isEqualTo(0);
      Assertions.assertThat(expiry.getAction()).isEqualTo(ExpirationAction.DESTROY);
    });
  ...
  public void alterExpirationAttributesWithStatisticsEnabled() {
    ...
    server1.invoke(() -> {
      ...
      Assertions.assertThat(expiry.getTimeout()).isEqualTo(0);
      Assertions.assertThat(expiry.getAction()).isEqualTo(ExpirationAction.INVALIDATE);
      Assertions.assertThat(region.getAttributes().getCustomEntryIdleTimeout().getClass().getName())
  ...
  public void alterEvictionMaxOnRegionWithoutEvictionAttributesHasNoEffect() throws Exception {
    ...
    locator.invoke(() -> {
      ...
      Assertions.assertThat(evictionAttributes).isNull();
    });

    server1.invoke(() -> {
      Region region = ClusterStartupRule.getCache().getRegion("/regionA");
      EvictionAttributes evictionAttributes = region.getAttributes().getEvictionAttributes();
      Assertions.assertThat(evictionAttributes.getAlgorithm()).isEqualTo(EvictionAlgorithm.NONE);
    });
  }
  ...
  public void alterRegionWithEvictionMaxOnRegionWithEviction() throws Exception {
    ...
    locator.invoke(() -> {
      ...
      RegionAttributesType.EvictionAttributes evictionAttributes =
          regionConfig.getRegionAttributes().getEvictionAttributes();
      Assertions.assertThat(evictionAttributes.getLruEntryCount().getMaximum()).isEqualTo("30");
      Assertions.assertThat(evictionAttributes.getLruEntryCount().getAction()).isEqualTo(
