...
public class RegionChangesPersistThroughClusterConfigurationDUnitTest {
  ...
  public void createdRegionPersistsThroughCacheConfig() {
    locator.invoke(() -> {
      ...
      Assertions.assertThat(sharedConfig.getConfiguration(GROUP_NAME).getCacheXmlContent())
    ...
    server2.invoke(() -> {
      ...
      Assertions.assertThat(cache.getRegionByPath(REGION_PATH)).isNotNull();
    });
  }
  ...
  public void regionUpdatePersistsThroughClusterConfig() {
    server2.invoke(() -> {
      InternalDistributedSystem system = InternalDistributedSystem.getConnectedInstance();
      InternalCache cache = system.getCache();
      Assertions.assertThat(cache.getRegionByPath(REGION_PATH).isEntryExpiryPossible()).isFalse();
    });
    ...
    locator.invoke(() -> {
      ...
      Assertions.assertThat(sharedConfig.getConfiguration(GROUP_NAME).getCacheXmlContent()).contains("45635");
    });
    ...
    server2.invoke(() -> {
      ...
      InternalCache cache = system.getCache();
      Assertions.assertThat(cache.getRegionByPath(REGION_PATH)).isNotNull();
      Assertions.assertThat(cache.getRegionByPath(REGION_PATH).isEntryExpiryPossible()).isTrue();
    });
  }
  ...
  public void destroyRegionPersistsThroughClusterConfig() {
    ...
    server2.invoke(() -> {
      InternalDistributedSystem system = InternalDistributedSystem.getConnectedInstance();
      InternalCache cache = system.getCache();
      Assertions.assertThat(cache.getRegionByPath(REGION_PATH)).isNull();
    });
  }
}
