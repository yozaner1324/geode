...
public class NetstatDUnitTest {
  ...
  public void testOutputToConsoleForAllMembers() throws Exception {
    ...
    Assertions.assertThat(lines.length).isGreaterThan(5);
    Assertions.assertThat(lines[4].trim().split("[,\\s]+")).containsExactlyInAnyOrder("locator-0",
        "server-1",
        "server-2");
  }
  ...
  public void testOutputToConsoleForOneMember() throws Exception {
    ...
    Assertions.assertThat(lines.length).isGreaterThan(5);
    Assertions.assertThat(lines[4].trim().split("[,\\s]+")).containsExactlyInAnyOrder("server-1");
  }
  ...
  public void testOutputToConsoleWithLsofForOneMember() throws Exception {
    ...
    Assertions.assertThat(lines.length).isGreaterThan(5);
    Assertions.assertThat(lines[4].trim().split("[,\\s]+")).containsExactlyInAnyOrder("server-1");
    Assertions.assertThat(lines).filteredOn(e -> e.contains("## lsof output ##")).hasSize(1);
  }
  ...
  public void testOutputToFile() throws Exception {
    ...
    Assertions.assertThat(lines.toString()).doesNotContain("Could not execute");

    Assertions.assertThat(lines.size()).isGreaterThan(5);
    Assertions.assertThat(lines.get(4).trim().split("[,\\s]+")).containsExactlyInAnyOrder("locator-0",
        "server-1", "server-2");
  }
  ...
  public void testOutputToFileForOneGroup() throws Exception {
    ...
    Assertions.assertThat(lines.size()).isGreaterThan(5);
    Assertions.assertThat(lines.get(4).trim().split("[,\\s]+")).containsExactly("server-1");
  }
  ...
  public void testOutputWithLsofToFile() throws Exception {
    ...
    Assumptions.assumeThat(isJavaVersionAtLeast(JavaVersion.JAVA_11))
