...
public class AlterCompressorDUnitTest {
  ...
  private static void verifyRegionIsCompressed() {
    ...
    Region testRegion = ClusterStartupRule.getCache().getRegion("testRegion");
    Assertions.assertThat(testRegion.getAttributes().getCompressor()).isNotNull();
    Assertions.assertThat(testRegion.getAttributes().getCompressor()).isInstanceOf(TestCompressor1.class);
    ...
    Assertions.assertThat(value).isInstanceOf(byte[].class);
    ...
    Assertions.assertThat(actual).isEqualTo(compressor.compress(expected));
  }

  private static void verifyRegionIsNotCompressed() {
    // assert the region attributes
    Region testRegion = ClusterStartupRule.getCache().getRegion("testRegion");
    Assertions.assertThat(testRegion.getAttributes().getCompressor()).isNull();

    // assert that values are not compressed
    LocalRegion localReg = ((LocalRegion) testRegion);
    RegionEntry entry = localReg.entries.getEntry("key19");
    Object value = entry.getValue();
    Assertions.assertThat(value).isInstanceOf(CachedDeserializable.class);
    CachedDeserializable expected =
        CachedDeserializableFactory.create(EntryEventImpl.serialize("value19"), null);
    Assertions.assertThat(((CachedDeserializable) value).getSerializedValue())
