...
public class DestroyIndexCommandsDUnitTest {
  ...
  public void testDestroyAllIndexesOnOneMember() throws Exception {
    ...
    server1.invoke(() -> {
      Cache cache = ClusterStartupRule.getCache();
      Assertions.assertThat(cache.getQueryService().getIndexes()).isEmpty();
    });

    server2.invoke(() -> {
      Cache cache = ClusterStartupRule.getCache();
      Assertions.assertThat(cache.getQueryService().getIndexes().size()).isEqualTo(2);
    });
  ...
  public void testDestroyOneIndexOnOneMember() throws Exception {
    ...
    server1.invoke(() -> {
      Cache cache = ClusterStartupRule.getCache();
      Assertions.assertThat(cache.getQueryService().getIndexes().size()).isEqualTo(1);
    });
  ...
  public void destroyIndexOnOneGroupWithoutAssociatedClusterConfig() {
    ...
    locator.invoke(() -> {
      ...
      Assertions.assertThat(regionConfig.getIndexes().size()).isEqualTo(2);
    });
  }
  ...
  private void assertIndexCount(String region, int indexCount) {
    server1.invoke(() -> {
      Cache cache = ClusterStartupRule.getCache();
      Assertions.assertThat(cache.getQueryService().getIndexes().size()).isEqualTo(indexCount);
    });
    ...
    locator.invoke(() -> {
      ...
      Assertions.assertThat(regionConfig.getIndexes().size()).isEqualTo(indexCount);
    });
  }
}
