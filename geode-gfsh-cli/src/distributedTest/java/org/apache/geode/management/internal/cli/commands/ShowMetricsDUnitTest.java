...
public class ShowMetricsDUnitTest {
  ...
  public void before() throws Exception {
    ...
    server.invoke(() -> {
      ...
      GeodeAwaitility.await().until(() -> isBeanReady(cache, 5, "", member, serverPort));
    });

    locator.invoke(() -> {
      Cache cache = ClusterStartupRule.getCache();
      // Wait for all of the relevant beans to be ready
      GeodeAwaitility.await().until(() -> isBeanReady(cache, 1, "", null, 0));
      GeodeAwaitility.await().until(() -> isBeanReady(cache, 2, "REGION1", null, 0));

      DistributedMember member = cache.getDistributedSystem().getDistributedMember();
      GeodeAwaitility.await().until(() -> isBeanReady(cache, 3, "", member, 0));
    });
  ...
  public void testShowMetricsRegion() throws Exception {
    ...
    Assertions.assertThat(gfsh.getGfshOutput()).contains("Cluster-wide Region Metrics");
  }
  ...
  public void testShowMetricsRegionFromMember() throws Exception {
    gfsh.executeAndAssertThat("show metrics --member=" + server.getName() + " --region=REGION1")
        .statusIsSuccess();
    Assertions.assertThat(gfsh.getGfshOutput()).contains("Metrics for region:/REGION1 On Member server-1");
  }
}
