...
public class AlterAsyncEventQueueCommandTest {
  ...
  @Before
  public void setUp() {
    command = Mockito.spy(AlterAsyncEventQueueCommand.class);
    service =
    ...
    groupSet.add("group1");
    groupSet.add("group2");
    Mockito.doReturn(groupSet).when(service).getGroups();
    ...
    Mockito.doReturn(config).when(service).getCacheConfig("group1");
    Mockito.doReturn(new CacheConfig()).when(service).getCacheConfig("group2");
    Mockito.doReturn(config).when(service).getCacheConfig("group1", true);
    Mockito.doReturn(new CacheConfig()).when(service).getCacheConfig("group2", true);

    Mockito.doReturn(true).when(service).lockSharedConfiguration();
    Mockito.doNothing().when(service).unlockSharedConfiguration();
    Mockito.doReturn(null).when(service).getConfiguration(ArgumentMatchers.any());
    Mockito.doReturn(configRegion).when(service).getConfigurationRegion();
