...
public class ListGatewayCommandTest {
  ...
  @Before
  public void setup() {
    command = Mockito.spy(ListGatewayCommand.class);
    member = Mockito.mock(DistributedMember.class);
    service = Mockito.mock(SystemManagementService.class);

    Mockito.doReturn(Stream.of(member).collect(toSet())).when(command).findMembers(ArgumentMatchers.any(), ArgumentMatchers.any());
    Mockito.doReturn(service).when(command).getManagementService();

    receiverMXBean = Mockito.mock(GatewayReceiverMXBean.class);
    Mockito.doReturn(5407).when(receiverMXBean).getPort();
    Mockito.doReturn(7).when(receiverMXBean).getClientConnectionCount();
  ...
  public void listGatewaysDisplaysGatewaySendersAndReceivers() {
    ...
    Mockito.doReturn(new String[] {"10.118.19.31(server-ny-2:33256)<v2>:1029",
