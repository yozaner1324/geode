...
public class ConfigurePDXCommandTest {
  ...
  public void setUp() throws Exception {
    ...
    command = Mockito.spy(ConfigurePDXCommand.class);
    clusterConfigurationService = Mockito.mock(InternalConfigurationPersistenceService.class);

    Mockito.doReturn(cache).when(command).getCache();
    Mockito.doReturn(Collections.emptySet()).when(command).getAllNormalMembers();
    Mockito.doReturn(clusterConfigurationService).when(command).getConfigurationPersistenceService();
  }
  ...
  public void parsingAutoCompleteShouldSucceed() throws Exception {
    ...
    Assertions.assertThat(candidate.getCandidates()).isNotNull();
    Assertions.assertThat(candidate.getCandidates().size()).isEqualTo(5);
  }
  ...
  public void executionShouldHandleInternalFailures() {
    ...
    Mockito.doThrow(new RuntimeException("Can't create ReflectionBasedAutoSerializer.")).when(command)
        .createReflectionBasedAutoSerializer(ArgumentMatchers.anyBoolean(), ArgumentMatchers.any());
    gfshParserRule
