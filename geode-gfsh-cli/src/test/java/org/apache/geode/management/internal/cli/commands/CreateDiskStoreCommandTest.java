...
public class CreateDiskStoreCommandTest {
  ...
  public void before() throws Exception {
    command = Mockito.spy(CreateDiskStoreCommand.class);
  }
  ...
  public void dirWithRelativePath() throws Exception {
    ...
    Mockito.doReturn(Collections.singletonList(Mockito.mock(CliFunctionResult.class))).when(command)
        .executeAndGetFunctionResult(ArgumentMatchers.any(), ArgumentMatchers.any(), ArgumentMatchers.any());
    Mockito.doReturn(Pair.of(Boolean.TRUE, null)).when(command).validateDiskstoreAttributes(ArgumentMatchers.any(),
        ArgumentMatchers.any());
    Mockito.doReturn(true).when(command).waitForDiskStoreMBeanCreation(ArgumentMatchers.any(), ArgumentMatchers.any());
    ...
    Assertions.assertThat(diskDirType.getContent().replace('\\', '/')).isEqualTo("./data/persist");
  }
