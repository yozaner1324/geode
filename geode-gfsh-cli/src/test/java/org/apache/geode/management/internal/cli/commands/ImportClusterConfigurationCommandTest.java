...
public class ImportClusterConfigurationCommandTest {
  ...
  public void setUp() throws Exception {
    command = Mockito.spy(ImportClusterConfigurationCommand.class);
    ...
    Mockito.doReturn(true).when(command).isSharedConfigurationRunning();
    Mockito.doReturn(ccService).when(command).getConfigurationPersistenceService();
    Mockito.doNothing().when(command).backupTheOldConfig(ccService);
    Mockito.doReturn(Collections.emptySet()).when(command).findMembers(ArgumentMatchers.any());
    Mockito.doReturn(xmlFile).when(command).getUploadedFile();
  ...
  public void autoComplete() {
    ...
    Assertions.assertThat(commandCandidate.getCandidates()).hasSize(2);
    Assertions.assertThat(commandCandidate.getFirstCandidate())
  ...
  public void noMemberFound() throws IOException {
    ...
    Assertions.assertThat(configuration.getCacheXmlContent()).isEqualTo(xmlContent);
  }
