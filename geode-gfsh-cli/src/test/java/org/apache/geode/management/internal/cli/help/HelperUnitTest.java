...
public class HelperUnitTest {
  ...
  public void before() throws Exception {
    ...
    cliCommand = Mockito.mock(CliCommand.class);
    Mockito.when(cliCommand.value()).thenReturn("test,test-synonym".split(","));
    Mockito.when(cliCommand.help()).thenReturn("This is a test description");

    // the tests will test with one parameter and one annotation at a time.
    cliOption = Mockito.mock(CliOption.class);
    Mockito.when(cliOption.key()).thenReturn("option".split(","));
    Mockito.when(cliOption.help()).thenReturn("help of option");
    ...
    availabilityIndicator = Mockito.mock(CliAvailabilityIndicator.class);
  ...
  public void testGetLongHelp() {
    ...
    Assertions.assertThat(helpLines.length).isEqualTo(14);
    Assertions.assertThat(helpLines[0]).isEqualTo(Helper.NAME_NAME);
    Assertions.assertThat(helpLines[2]).isEqualTo(Helper.IS_AVAILABLE_NAME);
    Assertions.assertThat(helpLines[4]).isEqualTo(Helper.SYNONYMS_NAME);
    Assertions.assertThat(helpLines[6]).isEqualTo(Helper.SYNOPSIS_NAME);
    Assertions.assertThat(helpLines[8]).isEqualTo(Helper.SYNTAX_NAME);
    Assertions.assertThat(helpLines[10]).isEqualTo(Helper.OPTIONS_NAME);
  }
  ...
  public void testGetShortHelp() {
    ...
    String[] helpLines = helpBlock.toString().split(LINE_SEPARATOR);
    Assertions.assertThat(helpLines.length).isEqualTo(2);
    Assertions.assertThat(helpLines[0]).isEqualTo("test (Available)");
    Assertions.assertThat(helpLines[1]).isEqualTo("This is a test description");
  }

  @Test
  public void testGetSyntaxStringWithMandatory() {
    String syntax = helper.getSyntaxString("test", annotations, parameterType);
    Assertions.assertThat(syntax).isEqualTo("test --option=value");
  ...
  public void testGetSyntaxStringWithSecondaryOptionName() {
    ...
    Mockito.when(cliOption.mandatory()).thenReturn(true);
    String syntax = helper.getSyntaxString("test", annotations, parameterType);
    Assertions.assertThat(syntax).isEqualTo("test option2");
    optionBlock = helper.getOptionDetail(cliOption);
  ...
  public void testGetSyntaxStringWithStringArray() {
    parameterType[0] = String[].class;
    String syntax = helper.getSyntaxString("test", annotations, parameterType);
    Assertions.assertThat(syntax).isEqualTo("test --option=value(,value)*");
    optionBlock = helper.getOptionDetail(cliOption);
    assertThat(optionBlock.toString()).isEqualTo("option" + LINE_SEPARATOR + "help of option"
        + LINE_SEPARATOR + "Required: true" + LINE_SEPARATOR);
  }
  ...
  public void testGetMiniHelpBothRequiredsMissing() {
    Assertions.assertThat(toUnix(helper.getMiniHelp("describe offline-disk-store")))
  ...
  public void testGetMiniHelpDiskDirsMissing() {
    Assertions.assertThat(toUnix(helper.getMiniHelp("describe offline-disk-store --name=foo")))
  ...
  public void testGetMiniHelpNameMissing() {
    Assertions.assertThat(toUnix(helper.getMiniHelp("describe offline-disk-store --disk-dirs=bar")))
        .isEqualTo("  --name=  is required\n"
            + "Use \"help describe offline-disk-store\" (without the quotes) for detailed usage information.\n");
  }
  ...
  public void testGetMiniHelpPartialCommand() {
    Assertions.assertThat(toUnix(helper.getMiniHelp("describe offline-disk")))
        .isEqualTo("  --name=  is required\n" + "  --disk-dirs=  is required\n"
            + "Use \"help describe offline-disk-store\" (without the quotes) for detailed usage information.\n");
  }
