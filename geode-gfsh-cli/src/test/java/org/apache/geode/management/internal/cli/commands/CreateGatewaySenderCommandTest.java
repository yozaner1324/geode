...
public class CreateGatewaySenderCommandTest {
  ...
  @Before
  public void before() {
    command = Mockito.spy(CreateGatewaySenderCommand.class);
    InternalCache cache = Mockito.mock(InternalCache.class);
    Mockito.doReturn(cache).when(command).getCache();
    Mockito.doReturn(true).when(command).waitForGatewaySenderMBeanCreation(ArgumentMatchers.any(), ArgumentMatchers.any());
    functionResults = new ArrayList<>();
    Mockito.doReturn(functionResults).when(command).executeAndGetFunctionResult(ArgumentMatchers.any(), ArgumentMatchers.any(), ArgumentMatchers.any());
  }
  ...
  public void orderPolicyAutoComplete() {
    ...
    Assertions.assertThat(candidate.getCandidates()).hasSize(3);
    Assertions.assertThat(candidate.getFirstCandidate()).isEqualTo(command + "=KEY");
  }
  ...
  public void testFunctionArgs() {
    ...
    gfsh.executeAndAssertThat(command,
        ...
        .statusIsSuccess();
    Mockito.verify(command).executeAndGetFunctionResult(ArgumentMatchers.any(), argsArgumentCaptor.capture(),
  ...
  public void testReturnsConfigInResultModel() {
    ...
    Assertions.assertThat(sender.getId()).isEqualTo("1");
    Assertions.assertThat(sender.getRemoteDistributedSystemId()).isEqualTo("1");
    Assertions.assertThat(sender.getOrderPolicy()).isEqualTo("THREAD");
  }
  ...
  public void whenMembersAreDifferentVersions() {
    ...
    Mockito.when(currentVersionMember.getVersionObject()).thenReturn(Version.CURRENT);
    InternalDistributedMember oldVersionMember = Mockito.mock(InternalDistributedMember.class);
    Mockito.when(oldVersionMember.getVersionObject()).thenReturn(Version.GEODE_1_4_0);
  ...
  public void testDefaultArguments() {
    ...
    gfsh.executeAndAssertThat(command,
        ...
        .statusIsSuccess();
    Mockito.verify(command).executeAndGetFunctionResult(ArgumentMatchers.any(), argsArgumentCaptor.capture(), ArgumentMatchers.any());
