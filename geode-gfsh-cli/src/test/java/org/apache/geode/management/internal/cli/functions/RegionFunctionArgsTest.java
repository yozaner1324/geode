...
public class RegionFunctionArgsTest {
  ...
  public void evictionAttributes() throws Exception {
    ...
    Assertions.assertThat(attributes.getAlgorithm()).isEqualTo(EvictionAlgorithm.LRU_HEAP);
    Assertions.assertThat(attributes.getAction()).isEqualTo(EvictionAction.LOCAL_DESTROY);
    Assertions.assertThat(attributes.getMaximum()).isEqualTo(0);

    args.setEvictionAttributes("overflow-to-disk", 1000, null, null);
    EvictionAttributes attributes1 = args.getEvictionAttributes()
        .convertToEvictionAttributes();
    Assertions.assertThat(attributes1.getAlgorithm()).isEqualTo(EvictionAlgorithm.LRU_MEMORY);
    Assertions.assertThat(attributes1.getAction()).isEqualTo(EvictionAction.OVERFLOW_TO_DISK);
    Assertions.assertThat(attributes1.getMaximum()).isEqualTo(1000);

    args.setEvictionAttributes("local-destroy", null, 1000, null);
    EvictionAttributes attributes2 = args.getEvictionAttributes()
        .convertToEvictionAttributes();
    Assertions.assertThat(attributes2.getAlgorithm()).isEqualTo(EvictionAlgorithm.LRU_ENTRY);
    Assertions.assertThat(attributes2.getAction()).isEqualTo(EvictionAction.LOCAL_DESTROY);
    Assertions.assertThat(attributes2.getMaximum()).isEqualTo(1000);
  }
