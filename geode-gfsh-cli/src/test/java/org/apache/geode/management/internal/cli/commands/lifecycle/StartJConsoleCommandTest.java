...
public class StartJConsoleCommandTest {
  ...
  public void setUp() throws Exception {
    command = Mockito.spy(StartJConsoleCommand.class);
    gfsh = Mockito.mock(Gfsh.class);
    doReturn(gfsh).when(command).getGfsh();
    Mockito.doReturn(null).when(command).getJmxServiceUrl();

    Process process = Mockito.mock(Process.class);
    Mockito.doReturn(process).when(command).getProcess(ArgumentMatchers.any(String[].class));
    Mockito.doReturn("some output").when(command).getProcessOutput(process);
  ...
  public void succcessOutput() throws Exception {
    ...
    Assertions.assertThat(commandString).hasSize(2);
    Assertions.assertThat(commandString[0]).contains("jconsole");
    Assertions.assertThat(commandString[1]).isEqualTo("-interval=4");
  }
  ...
  public void succcessOutputWithVersion() throws Exception {
    ...
    Assertions.assertThat(commandString).hasSize(2);
    Assertions.assertThat(commandString[0]).contains("jconsole");
    Assertions.assertThat(commandString[1]).isEqualTo("-version");
  }
  ...
  public void successWithServiceUrl() throws Exception {
    ...
    Mockito.verify(command, Mockito.times(1)).getProcess(argumentCaptor.capture());
    String[] commandString = argumentCaptor.getValue();
    Assertions.assertThat(commandString).hasSize(3);
    Assertions.assertThat(commandString[0]).contains("jconsole");
    Assertions.assertThat(commandString[1]).isEqualTo("-interval=4");
    Assertions.assertThat(commandString[2]).isEqualTo("service:jmx:rmi://localhost");
  }
}
