...
public class GatewaySenderDestroyFunctionTest {
  ...
  @Before
  public void before() {
    function = Mockito.spy(GatewaySenderDestroyFunction.class);
    context = Mockito.mock(FunctionContext.class);
    cache = Mockito.mock(InternalCache.class);
    args = Mockito.mock(GatewaySenderDestroyFunctionArgs.class);
    ...
    Mockito.when(context.getArguments()).thenReturn(args);
    Mockito.when(args.getId()).thenReturn("id");
    resultCaptor = ArgumentCaptor.forClass(CliFunctionResult.class);
    Mockito.when(cache.getDistributedSystem()).thenReturn(Mockito.mock(DistributedSystem.class));
  }
