...
public class ListJndiBindingCommandTest {
  ...
  public void setUp() throws Exception {
    command = Mockito.spy(ListJndiBindingCommand.class);
  ...
  public void noServiceNoMember() {
    ...
    Mockito.doReturn(Collections.emptySet()).when(command).findMembers(null, null);
  ...
  public void hasServiceNoBindingNoMember() {
    Mockito.doReturn(ccService).when(command).getConfigurationPersistenceService();
    Mockito.when(cacheConfig.getJndiBindings()).thenReturn(Collections.emptyList());
    Mockito.doReturn(Collections.emptySet()).when(command).findMembers(null, null);
