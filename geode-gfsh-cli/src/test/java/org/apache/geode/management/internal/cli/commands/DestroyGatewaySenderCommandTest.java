...
public class DestroyGatewaySenderCommandTest {
  ...
  public void before() throws Exception {
    command = Mockito.spy(DestroyGatewaySenderCommand.class);
    cache = Mockito.mock(InternalCache.class);
    Mockito.doReturn(cache).when(command).getCache();
    Mockito.doReturn(true).when(command).waitForGatewaySenderMBeanDeletion(ArgumentMatchers.any(), ArgumentMatchers.any());
