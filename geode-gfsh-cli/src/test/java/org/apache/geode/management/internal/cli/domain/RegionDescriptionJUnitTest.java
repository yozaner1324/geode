...
public class RegionDescriptionJUnitTest {
  ...
  public void findCommonRemovesUnsharedKeys() {
    ...
    Assertions.assertThat(commonMap).containsOnlyKeys(evictionKeyShared, partKeyShared, regKeyShared);
  }
  ...
  public void findCommonRemovesKeysWithDisagreeingValues() {
    ...
    Assertions.assertThat(commonMap).containsOnlyKeys(evictionKeyShared);
  }
  ...
  private RegionDescriptionPerMember getMockRegionDescriptionPerMember_A() {
    ...
    RegionDescriptionPerMember mockDescPerMember = Mockito.mock(RegionDescriptionPerMember.class);

    Mockito.when(mockDescPerMember.getNonDefaultEvictionAttributes())
        .thenReturn(mockNonDefaultEvictionAttributes);
    Mockito.when(mockDescPerMember.getNonDefaultPartitionAttributes())
        .thenReturn(mockNonDefaultPartitionAttributes);
    Mockito.when(mockDescPerMember.getNonDefaultRegionAttributes())
        .thenReturn(mockNonDefaultRegionAttributes);
    Mockito.when(mockDescPerMember.getHostingMember()).thenReturn("mockMemberA");
    Mockito.when(mockDescPerMember.getScope()).thenReturn(DISTRIBUTED_ACK);
    Mockito.when(mockDescPerMember.getDataPolicy()).thenReturn(NORMAL);
    Mockito.when(mockDescPerMember.getName()).thenReturn(regionName);
  ...
  private RegionDescriptionPerMember getMockRegionDescriptionPerMember_B() {
    ...
    Mockito.when(mockDescPerMember.getNonDefaultRegionAttributes())
        .thenReturn(mockNonDefaultRegionAttributes);
    Mockito.when(mockDescPerMember.getHostingMember()).thenReturn("mockMemberB");
    Mockito.when(mockDescPerMember.getScope()).thenReturn(DISTRIBUTED_ACK);
    Mockito.when(mockDescPerMember.getDataPolicy()).thenReturn(NORMAL);
    Mockito.when(mockDescPerMember.getName()).thenReturn(regionName);
  ...
  private RegionDescriptionPerMember getMockRegionDescriptionPerMember_OutOfScope() {
    ...
    Mockito.when(mockDescPerMember.getNonDefaultRegionAttributes())
        .thenReturn(mockNonDefaultRegionAttributes);
    Mockito.when(mockDescPerMember.getHostingMember()).thenReturn("mockMemberC");
    Mockito.when(mockDescPerMember.getScope()).thenReturn(LOCAL);
    Mockito.when(mockDescPerMember.getDataPolicy()).thenReturn(NORMAL);
    Mockito.when(mockDescPerMember.getName()).thenReturn(regionName);
