...
public class CreateRegionCommandTest {
  ...
  public void before() throws Exception {
    command = Mockito.spy(CreateRegionCommand.class);
    cache = Mockito.mock(InternalCache.class);
    Mockito.doReturn(cache).when(command).getCache();

    service = Mockito.mock(ManagementService.class);
    Mockito.doReturn(service).when(command).getManagementService();
    regionMXBean = Mockito.mock(DistributedRegionMXBean.class);
    Mockito.when(service.getDistributedRegionMXBean(ArgumentMatchers.any())).thenReturn(regionMXBean);
  }
  ...
  public void parseDeclarableWithClassOnly() {
    ...
    Assertions.assertThat(writer.getClassName()).isEqualTo("my.abc");
    Assertions.assertThat(writer.getInitProperties()).isNotNull().isEmpty();
  }
  ...
  public void parseDeclarableWithClassAndProps() {
    ...
    ClassName writer = (ClassName) result.getParamValue("cache-writer");
    Assertions.assertThat(writer.getClassName()).isEqualTo("my.abc");
    Assertions.assertThat(writer.getInitProperties()).containsKeys("k1", "k2");
  }
  ...
  public void parseDeclarableWithJsonWithSpace() {
    ...
    ClassName writer = (ClassName) result.getParamValue("cache-writer");
    Assertions.assertThat(writer.getClassName()).isEqualTo("my.abc");
    Assertions.assertThat(writer.getInitProperties()).containsOnlyKeys("k1", "k2").containsEntry("k1",
        "v   1");
  }
  ...
  public void cacheListenerClassOnly() {
    ...
    Assertions.assertThat(listeners).hasSize(2).contains(new ClassName("my.abc"), new ClassName("my.def"));
  }
  ...
  public void cacheListenerClassAndProps() {
    ...
    ClassName[] listeners = (ClassName[]) result.getParamValue("cache-listener");
    Assertions.assertThat(listeners).hasSize(2).contains(new ClassName("my.abc", json1),
