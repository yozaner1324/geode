...
public class DescribeRegionJUnitTest {
  ...
  @Before
  public void setup() {
    command = Mockito.spy(DescribeRegionCommand.class);
    functionResults = new ArrayList<>();
    Mockito.doReturn(functionResults).when(command).getFunctionResultFromMembers(ArgumentMatchers.any());
  }
  ...
  private RegionDescriptionPerMember createRegionDescriptionPerMember(String memberName,
    ...
    RegionDescriptionPerMember descriptionPerMember = Mockito.mock(RegionDescriptionPerMember.class);
    Mockito.when(descriptionPerMember.getNonDefaultEvictionAttributes()).thenReturn(evictionMap);
    Mockito.when(descriptionPerMember.getNonDefaultPartitionAttributes()).thenReturn(partitionMap);
    Mockito.when(descriptionPerMember.getNonDefaultRegionAttributes()).thenReturn(regionMap);
    Mockito.when(descriptionPerMember.getHostingMember()).thenReturn(memberName);
    Mockito.when(descriptionPerMember.getScope()).thenReturn(DISTRIBUTED_ACK);
    Mockito.when(descriptionPerMember.getDataPolicy()).thenReturn(NORMAL);
    Mockito.when(descriptionPerMember.getName()).thenReturn(regionName);
  ...
  static List<String> extractHostingMembers(Map<String, String> map) {
    ...
    Assertions.assertThat(map).containsKeys(key);
