...
public class GetRegionsFunctionTest {
  ...
  public void setUp() {
    ...
    Mockito.when(functionContext.<RegionInformation[]>getResultSender()).thenReturn(resultSender);
  }
  ...
  public void lastResultHasRegionInformationForRegion() {
    ...
    Assertions.assertThat(lastResultFrom(resultSender).getValue())
  ...
  private Cache cacheWithOneRootRegion(String regionName) {
    ...
    Mockito.when(regionAttributes.getDataPolicy()).thenReturn(Mockito.mock(DataPolicy.class));
    Mockito.when(regionAttributes.getScope()).thenReturn(Mockito.mock(Scope.class));
    Mockito.when(region.getFullPath()).thenReturn(Region.SEPARATOR + regionName);
    Mockito.when(region.subregions(ArgumentMatchers.anyBoolean())).thenReturn(Collections.emptySet());
    Mockito.when(region.getAttributes()).thenReturn(regionAttributes);
    Mockito.when(cache.rootRegions()).thenReturn(Collections.singleton(region));
    return cache;
  }
  ...
  private ArgumentCaptor<RegionInformation[]> lastResultFrom(
    ...
    Mockito.verify(resultSender).lastResult(lastResult.capture());
    return lastResult;
  }
}
