...
public class LogSizerTest {
  ...
  public void noFiles_returnsZeroSize() throws Exception {
    File mockStatFile = Mockito.mock(File.class);
    File mockLogFile = Mockito.mock(File.class);
    Mockito.when(mockLogFile.toPath()).thenReturn(
        new File("root" + separator + "parent" + separator + testName + ".log").toPath());
    Mockito.when(mockStatFile.toPath()).thenReturn(
