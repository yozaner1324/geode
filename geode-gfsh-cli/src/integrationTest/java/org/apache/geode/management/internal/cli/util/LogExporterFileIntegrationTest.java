...
public class LogExporterFileIntegrationTest {
  ...
  public void setup() throws Exception {
    logFilter = Mockito.mock(LogFilter.class);

    Mockito.when(logFilter.acceptsFile(ArgumentMatchers.any())).thenReturn(true);
    Mockito.when(logFilter.acceptsLine(ArgumentMatchers.any())).thenReturn(LogFilter.LineFilterResult.LINE_ACCEPTED);
  ...
  public void exporterShouldStillReturnFileIfNoAcceptableLogs() throws Exception {
    ...
    Mockito.when(logFilter.acceptsLine(ArgumentMatchers.any())).thenReturn(LogFilter.LineFilterResult.LINE_REJECTED);
    Path exportedZip = logExporter.export();
    Assertions.assertThat(exportedZip).isNotNull();
  ...
  public void exportBuildsZipCorrectlyWithTwoLogFiles() throws Exception {
    ...
    Assertions.assertThat(unzippedExportDir.listFiles()).hasSize(2);
    List<File> exportedFiles = Stream.of(unzippedExportDir.listFiles())
        .sorted(Comparator.comparing(File::getName)).collect(toList());

    Assertions.assertThat(exportedFiles.get(0)).hasSameContentAs(logFile1);
    Assertions.assertThat(exportedFiles.get(1)).hasSameContentAs(logFile2);
  }
