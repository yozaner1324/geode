...
public class GfshParserConverterTest {
  ...
  public void testUnspecifiedValueToStringArray() {
    ...
    Assertions.assertThat(memberIdValue).isNull();
  }
  ...
  public void testHelpConverterWithNo() {
    ...
    Assertions.assertThat(commandCandidate.size()).isEqualTo(commands.size());
  }
  ...
  public void testHelpConverter() {
    ...
    commandCandidate = parser.complete(command);
    Assertions.assertThat(commandCandidate.size()).isEqualTo(1);
    Assertions.assertThat(commandCandidate.getFirstCandidate()).isEqualTo(command + "ect");
  }
  ...
  public void testHintConverter() {
    ...
    Assertions.assertThat(commandCandidate.size()).isEqualTo(topics.size());
    Assertions.assertThat(commandCandidate.getFirstCandidate()).isEqualTo("hint --topic=Client");
  }
  ...
  public void testDiskStoreNameConverter() throws Exception {
    ...
    Mockito.doReturn(diskStores).when(spy).getCompletionValues();

    String command = "compact disk-store --name=";
    commandCandidate = parser.complete(command);
    Assertions.assertThat(commandCandidate.size()).isEqualTo(2);
  ...
  public void testFilePathConverter() throws Exception {
    ...
    Mockito.doReturn(roots).when(spy).getRoots();
    Mockito.doReturn(siblings).when(spy).getSiblings(ArgumentMatchers.any());

    String command = "start server --cache-xml-file=";
    commandCandidate = parser.complete(command);
    Assertions.assertThat(commandCandidate.size()).isEqualTo(2);
    Assertions.assertThat(commandCandidate.getFirstCandidate()).isEqualTo(command + "/logs");

    command = "start server --cache-xml-file=sibling";
    commandCandidate = parser.complete(command);
    Assertions.assertThat(commandCandidate.size()).isEqualTo(2);
    Assertions.assertThat(commandCandidate.getFirstCandidate()).isEqualTo(command + "1");
  ...
  public void testRegionPathConverter() throws Exception {
    ...
    Mockito.doReturn(regions).when(spy).getAllRegionPaths();

    String command = "describe region --name=";
    commandCandidate = parser.complete(command);
    Assertions.assertThat(commandCandidate.size()).isEqualTo(regions.size());
    Assertions.assertThat(commandCandidate.getFirstCandidate()).isEqualTo(command + "/regionA");
  }
  ...
  public void testExpirationAction() {
    ...
    commandCandidate = parser.complete(command);
    Assertions.assertThat(commandCandidate.size()).isEqualTo(4);
    Assertions.assertThat(commandCandidate.getFirstCandidate()).isEqualTo(command + "DESTROY");
  ...
  public void testJarFilesPathConverter() {
    ...
    commandCandidate = parser.complete(command);
    Assertions.assertThat(commandCandidate.size()).isGreaterThan(0);
  ...
  private void assertCandidateEndsWithFirstRoot(String candidate, String command) {
    ...
    Assertions.assertThat(candidate).isEqualTo(command + roots[0]);
  }
}
