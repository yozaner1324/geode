...
public class ServiceRegistryImpl implements ServiceRegistry {
  ...
  public <T> ServiceResult<T> getService(Class<T> clazz) {
    List<Object> matchingServices =
        services.values().stream().filter(service -> clazz.isAssignableFrom(service.getClass()))
            .collect(
                Collectors.toList());

    if (matchingServices.isEmpty()) {
      return Failure.of("No service found for type: " + clazz.getName());
    }
